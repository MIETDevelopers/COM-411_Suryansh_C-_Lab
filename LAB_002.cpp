/*Write a program to allocate appropriate access specifiers to data members of student and course class along with justification in comments*/
// Author:Suryansh Sharma
#include <iostream>
using namespace std;
class student
{ // data members are private and are only accessible to class member function
private:
    string name;
    int roll_no;
    float mmarks, smarks, emarks;

public:                                                 // public member function they can be called outside the class using class object
...
private:
    ...
    string code;
    string in_charge;
    // public member function  can be accessed outside the class using class object
public:
...
int main()
{
    student s("ram", 3, 4, 5, 6);
    student s1("sham", 5, 6, 4, 4);
    s.print();
    s.avg();
    s1.print();
    s1.avg();
    cout << endl
         << "course details:-" << endl;
    course c("OOPS using c++", "COM-411", "Arjun");
    course c1("RDBMS lab", "COM-412", "Navin");
    c.display();
    cout << endl;
    c1.display();
